<?php
    ...
    try {
        ...
        if ($search_keyword) {
            $stmt = $conn->prepare("SELECT * FROM store_products WHERE product_name LIKE ?");
...
<body class="bg-gray-100 mx-auto w-full md:w-2/3">
    ...
    <div class="category-list-header" id="categoryHeader">
        ...
        <div class="container px-4 pt-4 pb-2">
            <div class="overflow-x-scroll whitespace-nowrap" id="categoryList"
                ...
                <a class="category-btn mb-3 inline-block py-1 px-4 border-2 border-red-500 rounded-full mr-2 <?php echo $index === 0 ? 'bg-red-100 text-red-500' : 'bg-white text-black'; ?> cursor-pointer hover:bg-red-100 hover:text-red-500 font-medium"
                    aria-label="View <?php echo htmlspecialchars($category['name']); ?> category"
    ...
    <div id="product-list" class="mx-4 md:mx-0">
        ...
        <div class="category-section" data-category="<?php echo htmlspecialchars($category['name']); ?>"
            data-category-id="<?php echo $category['id']; ?>">
            <div class="container px-4 py-2 bg-gray-100">
                <div class="flex items-center justify-center" style="font-family: Khmer OS Battambang;">
                    ...
                    <span
                        class="px-3 text-black font-bold text-lg"><?php echo htmlspecialchars($category['name']); ?></span>
                    <div class="flex-grow border-t-4 border-gray-300 mr-6 sm:mr-12"></div>
                </div>
            </div>
        </div>
        <div class="grid grid-cols-2 gap-4 mt-0 md:mt-2 mx-2 product-section"
            data-category="<?php echo htmlspecialchars($category['name']); ?>"
            data-category-id="<?php echo $category['id']; ?>">
            ...
            <a href="#" class="bg-white shadow rounded-lg product-card category-<?php echo $product['category_id']; ?>"
                ...
                <img src="<?php echo $product['image']; ?>"
                    ...
                    class="w-full h-[150px] sm:h-[450px] mb-2 rounded-tl-lg rounded-tr-lg border" loading="lazy"
                    onerror="this.onerror=null; this.src='uploads/default-placeholder.png';">
                <p class="text-gray-500 text-xs font-bold mb-1 px-4">ID:
                    <?php echo htmlspecialchars($product['product_code']); ?></p>
                <p class="text-sm font-bold mb-6 px-4" style="font-family: Khmer OS Battambang;">
    ...
    <div id="productModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white rounded-lg p-6 max-w-2xl w-full shadow-lg relative">
            <div id="productGallery" class="relative w-full h-96 overflow-hidden rounded">
                <img id="modalImage" src="" class="w-full h-full object-cover">
                <button id="prevImage"
                    class="absolute left-0 top-1/2 transform -translate-y-1/2 bg-gray-800 text-white px-2 py-1 rounded">‹</button>
                <button id="nextImage"
                    class="absolute right-0 top-1/2 transform -translate-y-1/2 bg-gray-800 text-white px-2 py-1 rounded">›</button>
            </div>
            <button id="closeModal" style="border: solid 1px red;"
                class="absolute top-3 right-3 bg-red-700 bg-opacity-50 rounded-lg w-8 h-8 flex items-center justify-center">
            ...
            <p id="modalTitle" class="text-gray-600 text-sm mt-1"
                style="font-family: Khmer OS Battambang; font-weight: bold; font-size: 15px;"></p>
            ...
            <div class="flex justify-center items-center mt-4 space-x-6 text-2xl">
                <div class="flex space-x-2">
                    <a id="modalFacebook" href="#" class="text-blue-500 text-3xl"><i
                            class="fab fa-facebook"></i></a>&nbsp;&nbsp;
                    <a id="modalTelegram" href="#" class="text-blue-500 text-3xl"><i
    ...
    <footer class="p-4 bg-gray-200 mt-6">
        <p class="text-center text-gray-600">© 2025 <a href="https://fb.com/khmerpos2"
                class="text-blue-500 hover:underline">Simple POS</a> All rights reserved.</p>
    </footer>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function handleCategoryClick(e) {
        ...
        if (categoryHeader) {
            ...
            const yOffset = categoryHeader.getBoundingClientRect().top + window.scrollY -
                headerHeight + 5;
            window.scrollTo({
            ...
            if (searchTerm) {
                productSections.forEach(section => {
                    ...
                    products.forEach(product => {
                        const productName = product.getAttribute(
                                "data-name")
                            .toLowerCase();
                ...
                productSections.forEach(section => {
                    ...
                    section.querySelectorAll('.product-card').forEach(
                        product => {
                            product.style.display = "block";
                        });
                });
